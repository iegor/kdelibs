...
/* Mountain */
#include "mountain.moc"

#include "gradients.h"
#include "colorutil.h"
#include "mountainrc.h"
#include "mountainimage.h"

#include "bitmaps.h"
#include "pixmaploader.h"

#define loader Mountain::PixmapLoader::the()
...
namespace Mountain {
  ...
  class MountainStylePlugin : public QStylePlugin {
    public:
      MountainStylePlugin() {}
      ~MountainStylePlugin() {}
      ...
      QStringList keys() const {
        ...
        { return QStringList() << "Mountain"; }
        else
        { return QStringList(); }
      }

      QStyle* create(const QString& key) {
        if(key == "mountain") { return new MountainStyle(); }
        return 0;
      }
  };

  KDE_Q_EXPORT_PLUGIN(MountainStylePlugin);
  // ---------------------------------------------------
  ...
  void drawMountainArrow(QPainter* p, QColorGroup cg, QRect r, QStyle::PrimitiveElement pe, bool down, bool enabled) {
    QPointArray a;

    switch(pe) {
      case QStyle::PE_ArrowUp: {
        a.setPoints(QCOORDARRLEN(mountain_up_arrow), mountain_up_arrow);
        break; }
      case QStyle::PE_ArrowDown: {
        a.setPoints(QCOORDARRLEN(mountain_down_arrow), mountain_down_arrow);
        break; }
      case QStyle::PE_ArrowLeft: {
        a.setPoints(QCOORDARRLEN(mountain_left_arrow), mountain_left_arrow);
        break; }
      default: {
        a.setPoints(QCOORDARRLEN(mountain_right_arrow), mountain_right_arrow);
        break; }
    }

    p->save();
    //if(down) { p->translate( pixelMetric( PM_ButtonShiftHorizontal ), pixelMetric( PM_ButtonShiftVertical ) ); }

    if(enabled) {
      //CHECKME: Why is the -1 needed?
      a.translate(r.x() + r.width() / 2 - 1, r.y() + r.height() / 2);

      if(!down) { p->setPen(cg.buttonText()); }
      else { p->setPen(cg.button()); }
  ...
  /** \fn MountainStyle::renderMenuBlendPixmap
  * NOTE: reimplemented from KStyle
  */
  void MountainStyle::renderMenuBlendPixmap(KPixmap& pix, const QColorGroup &cg, const QPopupMenu* /* popup */) const {
    QColor col = cg.button();

  #ifdef Q_WS_X11 // Only draw menu gradients on TrueColor, X11 visuals
    if(QPaintDevice::x11AppDepth() >= 24) {
      KPixmapEffect::gradient(pix, col.light(120), col.dark(115), KPixmapEffect::HorizontalGradient);
    }
    else
  #endif
      pix.fill(col);
  ...
  QRect MountainStyle::subRect(SubRect r, const QWidget *widget) const {
    ...
    switch(r) {
      ...
      case SR_CheckBoxFocusRect: {
        ...
        if(cb->text().isEmpty() && (cb->pixmap() == 0)) {
          ...
          QSize checkDim = loader.size(mountain_checkbox_on);
          int cw = checkDim.width();
          int ch = checkDim.height();

          QRect checkbox(bounding.x() + 1, bounding.y() + 1 + (bounding.height() - ch) / 2, cw - 3, ch - 4);
  ...
  QPixmap MountainStyle::stylePixmap(StylePixmap stylepixmap, const QWidget* widget, const QStyleOption& opt) const {
    switch(stylepixmap) {
      case SP_TitleBarMinButton:
        return Mountain::PixmapLoader::the().pixmap(mountain_title_iconify,
              Qt::black, Qt::black, false, false);
        //return qpixmap_from_bits( iconify_bits, "title-iconify.png" );
      case SP_TitleBarMaxButton:
        return Mountain::PixmapLoader::the().pixmap(mountain_title_maximize,
              Qt::black, Qt::black, false, false);
      case SP_TitleBarCloseButton:
        if(widget && widget->inherits("KDockWidgetHeader"))
          return Mountain::PixmapLoader::the().pixmap(mountain_title_close_tiny,
                Qt::black, Qt::black, false, false);
        else  return Mountain::PixmapLoader::the().pixmap(mountain_title_close,
                      Qt::black, Qt::black, false, false);
      case SP_TitleBarNormalButton:
        return Mountain::PixmapLoader::the().pixmap(mountain_title_restore,
              Qt::black, Qt::black, false, false);
      default:
        break;
    }
  ...
  , kickerMode(false) {
    ...
    highlightScrollBar = settings.readBoolEntry("/mountain/Settings/highlightScrollBar", true);
    animateProgressBar = settings.readBoolEntry("/mountain/Settings/animateProgressBar", false);
  ...
  MountainStyle::~MountainStyle() {
    Mountain::PixmapLoader::release();
    Mountain::GradientPainter::releaseCache();
    MountainDbCleanup();
  }

  /** \fn MountainStyle::updateProgressPos
   * \brief Update the registered progressbars.
   */
  void MountainStyle::updateProgressPos() {
    QMap<QProgressBar*, int>::iterator iter;
    bool visible = false;
    for(iter = progAnimWidgets.begin(); iter != progAnimWidgets.end(); ++iter) {
      QProgressBar* pbar = iter.key();
      if(pbar->isVisible() && pbar->isEnabled() && pbar->progress() != pbar->totalSteps()) {
        ++iter.data();
        if(iter.data() == 28) { iter.data() = 0; }
        iter.key()->update();
      }
      if(iter.key()->isVisible()) { visible = true; }
    }
    if(!visible) { animationTimer->stop(); }
  }

  void MountainStyle::polish(QApplication* app) {
    if(!qstrcmp(app->argv()[0], "kicker")) { kickerMode = true; }
  }

  void MountainStyle::polish(QWidget* widget) {
    ...
    else if(!qstrcmp(widget->name(), Mountain::kdeToolbarWidget)) {
      widget->setBackgroundMode(NoBackground);
      widget->installEventFilter(this);
    }
  ...
  void MountainStyle::unPolish(QWidget* widget) {
    //### TODO: This needs major cleanup (and so does polish() )
    if(widget->inherits("QPushButton") || widget->inherits("QComboBox")) {
      if(widget->inherits("QComboBox")) { widget->setBackgroundMode(PaletteButton); }
  ...
  void MountainStyle::progressBarDestroyed(QObject* obj) { progAnimWidgets.remove(static_cast<QProgressBar*>(obj)); }
  void MountainStyle::polish(QPalette&) { loader.clear(); }

  /** \fn renderToolbarEntryBackground
  * \brief Draws gradient background for toolbar buttons, handles and spacers
  */
  static void renderToolbarEntryBackground(QPainter* paint, const QToolBar* parent, QRect r,
                                           const QColorGroup& cg, bool horiz) {
    ...
    int xoff = 0, yoff = 0;
    if(horiz) { yoff = (toolHeight - r.height()) / 2; }
    else { xoff = (toolWidth - r.width()) / 2; }

    Mountain::GradientPainter::renderGradient(paint, r, cg.button(), horiz, false /*Not a menubar*/, xoff, yoff,
  ...
  static void renderToolbarWidgetBackground(QPainter* painter, const QWidget* widget) {
    ...
    if(tb) {
      ...
      if(tb->place() == QDockWindow::OutsideDock) {
        ...
        int needToTouchBottom = tb->height() - tb->frameWidth() - (widget->rect().bottom() + y_offset);
    ...
    if(painter) {
      Mountain::GradientPainter::renderGradient(painter, widget->rect(),
          widget->colorGroup().button(), horiz_grad, false,
          x_offset, y_offset, toolWidth, toolHeight);
    } else {
      QPainter p(widget);
      Mountain::GradientPainter::renderGradient(&p, widget->rect(),
          widget->colorGroup().button(), horiz_grad, false,
          x_offset, y_offset, toolWidth, toolHeight);
    }
  }
  ...
  void MountainStyle::drawPrimitive(PrimitiveElement pe, QPainter *p, const QRect &r, const QColorGroup &cg,
    ...
    switch(pe) {
      ...
      case PE_ButtonDefault: {
        ...
        int id;
        if(sunken) { id  = mountain_pushbutton_default_pressed; }
        else { id = mountain_pushbutton_default; }

        if((flags & Style_MouseOver) && (id == mountain_pushbutton_default)) { id = mountain_pushbutton_default_hov; }

        //p->fillRect( r, cg.background() );
        Mountain::RectTilePainter(id, false).draw(p, r, cg.button(), cg.background(), disabled,  pmode());
        break; }
      ...
      case PE_ButtonTool: {
        if(titleBarMode) {
          ...
          if(titleBarMode == Maximized) {
            ...
            if(flags & Style_Down) { offY += 1; }

            nr = QRect(r.x() + offX, r.y() + offY, 15, 15);
          }

          Mountain::ScaledPainter(flags & Style_Down ? mountain_titlebutton_pressed : mountain_titlebutton,
                                Mountain::ScaledPainter::Both).draw(p, nr, cg.button(), cg.background());
          return;
        }

        if(on) {
          Mountain::RectTilePainter(mountain_toolbar_clk).draw(p, r, cg.button(), cg.background());
          p->setPen(cg.dark());
          p->drawLine(x, y, x2, y);
          p->drawLine(x, y, x, y2);
        }
        else if(down) { Mountain::RectTilePainter(mountain_toolbar_clk).draw(p, r, cg.button(), cg.background()); }
        else {
          if(flags & Style_MouseOver) {
            Mountain::GradientPainter::renderGradient(p, QRect(r.x(), 0, r.width(), r.height()),
                Mountain::ColorUtil::lighten(cg.button(), 115), flags & Style_Horizontal, false);
          }
          else {
            Mountain::GradientPainter::renderGradient(p, QRect(r.x(), 0, r.width(), r.height()), cg.button(),
                flags & Style_Horizontal, false);
          }
          ...
          p->setPen(Mountain::ColorUtil::lighten(cg.button(), 115));
      ...
      case PE_ButtonCommand: {
        ...
        if(w <= smallButMaxW || h <= smallButMaxH || forceSmallMode) {
          if(sunken) { name = mountain_pushbutton_small_pressed; }
          else { name =  mountain_pushbutton_small; }
          forceSmallMode = false;
        }
        else {
          if(sunken) { name = mountain_pushbutton_pressed; }
          else { name =  mountain_pushbutton; }
        }

        if((flags & Style_MouseOver) && (name == mountain_pushbutton)) { name = mountain_pushbutton_hov; }
        ...
        if(toolbarBlendWidget && !flatMode) {
          ...
          Mountain::RectTilePainter(name, false).draw(p, r, cg.button(), cg.background(), disabled,
                                    Mountain::TilePainter::PaintFullBlend);
        }
        else if(!flatMode) {
          Mountain::RectTilePainter(name, false).draw(p, r, cg.button(), cg.background(), disabled, pmode()); }
        else {
          Mountain::ScaledPainter(name + MountainTileCC, Mountain::ScaledPainter::Vertical).
              draw( p, r, cg.button(), cg.background(), disabled, pmode());
      ...
      case PE_ButtonBevel: {
        ...
        if(w > 4 && h > 4) {
          if(sunken) { p->fillRect(x + 2, y + 2, w - 4, h - 4, cg.button()); }
          else {
            Mountain::GradientPainter::renderGradient(p, QRect(x + 2, y + 2, w - 4, h - 4), cg.button(),
      ...
      //honor it
        if(opt.isDefault()) { p->drawWinFocusRect(r); }
        else { p->drawWinFocusRect(r, opt.color()); }
        break; }
      ...
      case PE_HeaderSection: {
        if(flags & Style_Down) {
          Mountain::RectTilePainter(mountain_listview_pressed, false).draw(p, r, cg.button(), cg.background());
        }
        else {
          Mountain::RectTilePainter(mountain_listview, false).draw(p, r, cg.button(), cg.background());
        }
        break; }

      case PE_HeaderArrow: {
        if(flags & Style_Up) { drawPrimitive(PE_ArrowUp, p, r, cg, Style_Enabled); }
      ...
      case PE_ScrollBarSlider: {
        ...
        int name = MountainSlider1;
        unsigned int count = 3;

        if(horizontal) {
          if(w > (loader.size(mountain_scrollbar_hbar + MountainSlider1).width() +
                  loader.size(mountain_scrollbar_hbar + MountainSlider4).width() +
                  loader.size(mountain_scrollbar_hbar + MountainSlider3).width() + 2)) {
            count = 5;
          }
        }
        else if(h > (loader.size(mountain_scrollbar_vbar + MountainSlider1).height() +
                      loader.size(mountain_scrollbar_vbar + MountainSlider4).height() +
                      loader.size(mountain_scrollbar_vbar + MountainSlider3).height() + 2)) {
          count = 5;
        }
      ...
      case PE_ScrollBarAddLine: {
        ...
        if(flags & Style_Horizontal) {
          Mountain::CenteredPainter painter(mountain_scrollbar_hbar_arrow2);
          ...
          drawMountainArrow(p, cg, QRect(r.x(), r.y(), r.width() / 2, r.height()), PE_ArrowLeft, down, !disabled);

          drawMountainArrow(p, cg, QRect(r.x() + r.width() / 2, r.y(), r.width() - r.width() / 2, r.height()),
                          PE_ArrowRight, down, !disabled);
        }
        else {
          Mountain::CenteredPainter painter(mountain_scrollbar_vbar_arrow2);
          painter.draw(p, r, down ? cg.buttonText() : cg.button(), cg.background(), disabled, pmode());
          ...
          drawMountainArrow(p, cg, QRect(r.x(), r.y(), r.width(), r.height() / 2), PE_ArrowUp, down, !disabled);

          drawMountainArrow(p, cg, QRect(r.x(), r.y() + r.height() / 2, r.width(), r.height() - r.height() / 2),
                          PE_ArrowDown, down, !disabled);
          //drawMountainArrow(p, cg, r, PE_ArrowUp, down, !disabled);
        }
      ...
      case PE_ScrollBarSubLine: {
        ...
        if(flags & Style_Horizontal) {
          Mountain::CenteredPainter painter(mountain_scrollbar_hbar_arrow1);
          painter.draw(p, r, down ? cg.buttonText() : cg.button(), cg.background(), disabled, pmode());
          drawMountainArrow(p, cg, r, PE_ArrowLeft, down, !disabled);
        }
        else {
          Mountain::CenteredPainter painter(mountain_scrollbar_vbar_arrow1);
          painter.draw(p, r, down ? cg.buttonText() : cg.button(), cg.background(), disabled, pmode());
          drawMountainArrow(p, cg, r, PE_ArrowUp, down, !disabled);
        }
        break; }
      ...
      case PE_IndicatorMask: {
        if(flags & Style_On) {
          Mountain::ScaledPainter(mountain_checkbox_on).draw(p, r, cg.button(), cg.background(), disabled, pmode());
        }
        else if(flags & Style_Off) {
          Mountain::ScaledPainter(mountain_checkbox_off).draw(p, r, cg.button(), cg.background(), disabled, pmode());
        }
        else {
          Mountain::ScaledPainter(mountain_checkbox_tri).draw(p, r, cg.button(), cg.background(), disabled, pmode());
        }
        break; }
      ...
      case PE_ExclusiveIndicatorMask: {
        Mountain::ScaledPainter(on ? mountain_radiobutton_on : mountain_radiobutton_off).
            draw(p, r, cg.button(), cg.background(), disabled, pmode());
        break; }

      // LINEEDIT frame (exclusive indicator)
      // -------------------------------------------------------------------
      case PE_PanelLineEdit: {
        if(opt.isDefault() || opt.lineWidth() == 1) {
        //1-pixel frames can not be simply clipped wider frames, as that would produce too little contrast
        // on the lower border
          p->setPen(cg.dark());
      ...
      case PE_PanelMenuBar: { // Menu
        Mountain::GradientPainter::renderGradient(p, r, cg.button(), true, true);
        //Mountain::ScaledPainter( mountain_menubar , Mountain::ScaledPainter::Vertical).
        //  draw( p, r, cg.button(), cg.background() );
      ...
      case PE_PanelDockWindow: { // Toolbar
        ...
        if(lw) {
          ...
          QColor gradTop = Mountain::ColorUtil::lighten(cg.button(), 110);
          QColor gradBot = Mountain::ColorUtil::lighten(cg.button(), 109);
          if(horiz) {
            //Top line
            p->setPen(gradTop);
            p->drawLine(x, y, x2, y);
            ...
            //Left line
            Mountain::GradientPainter::renderGradient( p, QRect(r.x(), r.y(), 1, r.height()), cg.button(), true);
            ...
            //Top line
            Mountain::GradientPainter::renderGradient( p, QRect(r.x(), r.y(), r.width(), 1), cg.button(), false);
      ...
      case PE_DockWindowSeparator: {
        ...
        QToolBar* parent      = 0;
        if(paintWidget) { parent = ::qt_cast<QToolBar*>(paintWidget->parentWidget()); }
      ...
      default: {
        ...
        else { KStyle::drawPrimitive(pe, p, r, cg, flags, opt); }
      }
    }
  }

  void MountainStyle::drawKStylePrimitive(KStylePrimitive kpe, QPainter* p, const QWidget* widget, const QRect &r,
    ...
    switch(kpe) {
      ...
      case KPE_SliderGroove: {
        ...
        Mountain::TilePainter::PaintMode pmod = Mountain::TilePainter::PaintNormal;

        if(slider->erasePixmap() && !slider->erasePixmap()->isNull()) { pmod = Mountain::TilePainter::PaintFullBlend; }

        if(horizontal) {
          Mountain::RectTilePainter(mountain_slider_hgroove, false).draw(p, r, cg.button(), cg.background(), disabled,
                                    pmod);
        }
        else {
          Mountain::RectTilePainter(mountain_slider_vgroove, true, false).draw(p, r, cg.button(), cg.background(),
                                    disabled, pmod);
        }
        break; }
      ...
      case KPE_SliderHandle: {
        ...
        if(!disabled && flags & Style_Active) { hl = Mountain::ColorUtil::lighten(cg.highlight(), 110); }

        if(horizontal) {
          Mountain::ScaledPainter(mountain_slider).draw(p, r, disabled ? cg.button() : hl,
              Qt::black,  disabled, Mountain::TilePainter::PaintFullBlend);
        }
        else {
          Mountain::ScaledPainter(mountain_vslider).draw(p, r, disabled ? cg.button() : hl,
              Qt::black,  disabled, Mountain::TilePainter::PaintFullBlend);
        }
        break; }
  ...
  bool MountainStyle::isFormWidget(const QWidget* widget) const {
    ...
    if(!potentialClipPort || potentialClipPort->isTopLevel()) { return false; }
  ...
  void MountainStyle::drawControl(ControlElement element, QPainter *p, const QWidget *widget, const QRect &r,
                                const QColorGroup &cg, SFlags flags, const QStyleOption& opt) const {
    bool disabled = (flags & Style_Enabled) == 0;
    int x, y, w, h;
    r.rect(&x, &y, &w, &h);
    ...
    switch(element) {
      ...
      case CE_PushButton: {
        ...
        if(isFormWidget(btn)) { formMode = true; }
        if(widget == hoverWidget) { flags |= Style_MouseOver; }
        if(btn->isFlat()) { flatMode = true; }

        if(btn->isDefault() && !flatMode) { drawPrimitive(PE_ButtonDefault, p, r, cg, flags); }
        else {
          if(widget->parent() && widget->parent()->inherits("QToolBar")) { toolbarBlendWidget = widget; }
      ...
      case CE_PushButtonLabel: {
        ...
        if(button->isMenuButton()) {
          ...
          if(button->iconSet() && !button->iconSet()->isNull()  &&
            ...
            drawPrimitive(PE_ArrowDown, p, visualRect(QRect(x + w - dx - 8, y + 2, dx, h - 4), r), cg, flags, opt);
            w -= dx;
          }
        }
        ...
        if(button->iconSet() && !button->iconSet()->isNull()) {
          ...
          if(button->isEnabled()) { mode = button->hasFocus() ? QIconSet::Active : QIconSet::Normal; }
          if(button->isToggleButton() && button->isOn()) { state = QIconSet::On; }
          ...
          if(!button->text().isEmpty()) {
            ...
            int length = icon.width() + TextToIconMargin + p->fontMetrics().size(ShowPrefix, button->text()).width();
            //Calculate offset.
            ...
            //Icon only. Center it.
            if(!button->pixmap()) { p->drawPixmap(x + w / 2 - icon.width() / 2, y + h / 2 - icon.height() / 2, icon); }
        ...
        drawItem(p, QRect(x, y, w, h), AlignCenter | ShowPrefix, button->colorGroup(), button->isEnabled(),
                 button->pixmap(), button->text(), -1, &button->colorGroup().buttonText());
      ...
      case CE_ToolButtonLabel: {
        ...
        if(!onToolbar) {
          if(widget->parentWidget() && !qstrcmp(widget->parentWidget()->name(), "qt_maxcontrols")) {
      ...
      case CE_TabBarTab: {
        ...
        bool bottom = tabBar->shape() == QTabBar::RoundedBelow || tabBar->shape() == QTabBar::TriangularBelow;
        ...
        if(flags & Style_Selected) {
          ...
          if(tabBar->indexOf(opt.tab()->identifier()) != rightMost) { tabRect.setWidth(tabRect.width() + 1); }
          Mountain::ActiveTabPainter(bottom).draw(p, tabRect, cg.button().light(110), cg.background(),
                                     !tabBar->isEnabled(), pmode());
        }
        else {
          Mountain::InactiveTabPainter::Mode mode;
          int index = tabBar->indexOf(opt.tab()->identifier());
          if(index == 0) { mode = Mountain::InactiveTabPainter::First; }
          else if(index == tabBar->count() - 1) { mode = Mountain::InactiveTabPainter::Last; }
          else { mode = Mountain::InactiveTabPainter::Middle; }

          if(bottom) {
            Mountain::InactiveTabPainter(mode, bottom).draw(p, x, y, w, h - 3, cg.button(), cg.background(), disabled,
                                         pmode());
            p->setPen(cg.dark());
            p->drawLine(x, y, x + w, y);
          }
          else {
            Mountain::InactiveTabPainter(mode, bottom).draw(p, x, y + 3, w, h - 3, cg.button(), cg.background(),
                                         disabled, pmode());
            p->setPen(cg.light());
      ...
      case CE_DockWindowEmptyArea: {
        ...
        if(widget && widget->inherits("QToolBar")) {
          ...
          Mountain::GradientPainter::renderGradient(p, pr, cg.button(), tb->orientation() == Qt::Horizontal);
        }
        else { KStyle::drawControl(CE_DockWindowEmptyArea, p, widget, r, cg, flags, opt); }
        break; }
      ...
      case CE_MenuBarItem: {
        ...
        if(active && focused) {
          qDrawShadePanel(p, r.x(), r.y(), r.width(), r.height(), cg, true, 1, &cg.brush(QColorGroup::Midlight));
        }
        else { Mountain::GradientPainter::renderGradient(p, pr, cg.button(), true, true); }

        drawItem(p, r, AlignCenter|AlignVCenter|ShowPrefix|DontClip|SingleLine, cg, flags & Style_Enabled,
                 mi->pixmap(), mi->text());
      ...
      case CE_PopupMenuItem: {
        ...
        if(checkable) { checkcol = QMAX(checkcol, 20); }

        // Draw the menu item background
        if(active) {
          if(enabled) { Mountain::RowPainter(mountain_menuitem).draw(p, main, cg.highlight(), cg.background()); }
          else {
            if(widget->erasePixmap() && !widget->erasePixmap()->isNull()) {
        ...
        if(mi->iconSet()) {
          ...
          if(active) { mode = enabled ? QIconSet::Active : QIconSet::Disabled; }
          else { mode = enabled ? QIconSet::Normal : QIconSet::Disabled; }

          // Do we have an icon and are checked at the same time?
          // Then draw a "pressed" background behind the icon
          if(checkable && /*!active &&*/ mi->isChecked()) {
            qDrawShadePanel(p, cr.x(), cr.y(), cr.width(), cr.height(), cg, true, 1, &cg.brush(QColorGroup::Midlight));
          }
        ...
        int xp = reverse ? x + tab + rightBorder + itemHMargin + itemFrame - 1 : x + xm; // X position
        ...
        if(mi->custom()) {
          ...
          // The menu item doesn't have a text label. Check if it has a pixmap instead
          else if(mi->pixmap()) {
            QPixmap *pixmap = mi->pixmap();

            // Draw the pixmap
            if(pixmap->depth() == 1) { p->setBackgroundMode(OpaqueMode); }
            int diffw = ((w - pixmap->width()) / 2) + ((w - pixmap->width()) % 2);
        ...
        if(mi->popup()) {
          ...
          QRect vr = visualRect(QRect(x + w - arrowHMargin - itemFrame - dim, y + h / 2 - dim / 2, dim, dim), r);

          // Draw an arrow at the far end of the menu item
          if(active) {
            if(enabled) { discol = cg.buttonText(); }
            QColorGroup g2(discol, cg.highlight(), white, white, enabled ? white : discol, discol, white);
            drawPrimitive(arrow, p, vr, g2, Style_Enabled);
          } else { drawPrimitive(arrow, p, vr, cg, enabled ? Style_Enabled : Style_Default); }
        }
        break; }
      ...
      case CE_ProgressBarContents: {
        ...
        if(progress > 0 || steps == 0) {
          ...
          if(steps == 0) { width = QMIN(width, 20); } //Don't cross squares
          ...
          if(steps == 0) {
            ...
            QRect progressRect;
            if(reverse) {
              progressRect = QRect(cr.x() + cr.width() - width - pstep, cr.y(), width, cr.height());
            }
            else {
              progressRect = QRect(cr.x() + pstep, cr.y(), width, cr.height());
            }

            Mountain::RowPainter(mountain_progressbar).draw(p, progressRect, cg.highlight(), cg.background());
            return;
          }

          QRect progressRect;

          if(reverse) { progressRect = QRect(cr.x() + (cr.width() - width), cr.y(), width, cr.height()); }
          else { progressRect = QRect(cr.x(), cr.y(), width, cr.height()); }
          ...
          if(animateProgressBar) {
            ...
            if(reverse) {
              ...
              QSize   endDim = loader.size(mountain_progressbar + 3); //3 = 3*1 + 0 = (1,0) = cl
              ...
              if(endDim.width() < progressRect.width()) {
                ...
                QPixmap endline = loader.scale(mountain_progressbar + 3, endDim.width(), progressRect.height(),
                ...
                //Paint the stripes.
                QPixmap stripe = loader.scale(mountain_progressbar + 4, 28, progressRect.height(),
                                              cg.highlight(), cg.background());
                //4 = 3*1 + 1 = (1,1) = cc
          ...
          Mountain::ProgressBarPainter(mountain_progressbar, reverse).draw(p, progressRect , cg.highlight(),
                                       cg.background());
        }
        break; }
  ...
  void MountainStyle::drawControlMask(ControlElement element, QPainter *p, const QWidget *widget, const QRect &r,
  ...
  bool MountainStyle::isSizeConstrainedCombo(const QComboBox* combo) const {
    if(combo->width() >= 80) { return false; }
    int suggestedWidth = combo->sizeHint().width();

    if(combo->width() - suggestedWidth < -5) { return true; }

    return false;
  }

  void MountainStyle::drawComplexControl(ComplexControl control, QPainter *p, const QWidget *widget, const QRect &r,
                                        const QColorGroup &cg, SFlags flags, SCFlags controls, SCFlags active,
                                        const QStyleOption& opt) const {
    bool disabled = (flags & Style_Enabled) == 0;
    switch(control) {
      // COMBOBOX
      ...
      case CC_ComboBox: {
        ...
        if(isFormWidget(cb)) { formMode = true; }
        ...
        if(controls & SC_ComboBoxFrame) {
          if(toolbarMode) { toolbarBlendWidget = widget; }
          if(widget == hoverWidget) { flags |= Style_MouseOver; }
        ...
        if(controls & SC_ComboBoxArrow) {
          ...
          if(!compact) {
            ...
            QRect rr = visualRect(QRect(ar.x(), ar.y() + 4, loader.size(mountain_ripple).width(), ar.height() - 8),
                                  widget);
            ...
            Mountain::ScaledPainter(mountain_ripple).draw(p2, rr, cg.button(), Qt::black, disabled,
                                    Mountain::TilePainter::PaintFullBlend);
          }
        ...
        if(controls & SC_ComboBoxEditField) {
          if(cb->editable()) {
            ...
            if(compact) { re.addCoords(3, 3, 0, -3); }
            p2->fillRect(re, cg.brush(QColorGroup::Highlight));
            drawPrimitive(PE_FocusRect, p2, re, cg, Style_FocusAtBorder, QStyleOption(cg.highlight()));
          }
      ...
      case CC_SpinWidget: {
        ...
        if(controls & SC_SpinWidgetButtonField) {
          Mountain::SpinBoxPainter().draw(p, br, cg.button(), cg.background(), !sw->isEnabled());
          if(active & SC_SpinWidgetUp) {
            Mountain::CenteredPainter(mountain_spinbox_pressed_arrow_up).draw(p, br.x(), br.y() + 3, br.width(),
                                      br.height() / 2, cg.button(), cg.background());
          }
          else {
            Mountain::CenteredPainter(mountain_spinbox_arrow_up).draw(p, br.x(), br.y() + 3, br.width(),
                                      br.height() / 2, cg.button(), cg.background(), !sw->isUpEnabled());
          }

          if(active & SC_SpinWidgetDown) {
            Mountain::CenteredPainter(mountain_spinbox_pressed_arrow_down).draw(p, br.x(), br.y() + br.height() / 2,
                                      br.width(), br.height() / 2 - 8, cg.button(), cg.background());
          }
          else {
            Mountain::CenteredPainter(mountain_spinbox_arrow_down).draw(p, br.x(), br.y() + br.height() / 2, br.width(),
                                      br.height() / 2 - 8, cg.background(), cg.button(), !sw->isDownEnabled());
          }
        }

        if(controls & SC_SpinWidgetFrame) { drawPrimitive(PE_PanelLineEdit, p, r, cg); }
        break; }

      case CC_ScrollBar: {
        const QScrollBar* sb = static_cast< const QScrollBar* >(widget);
        //Don't do the check if not highlighting anyway
        if(highlightScrollBar && sb->parentWidget()) {
          if(sb->parentWidget()->colorGroup().button() != sb->colorGroup().button()) { customScrollMode = true; }
        }
        ...
        if(controls & SC_ScrollBarSubLine) {
          drawPrimitive(PE_ScrollBarSubLine, p, subline, cg, flags | ((active & SC_ScrollBarSubLine) ? Style_Down : 0));
        }

        QRegion clip;
        if(controls & SC_ScrollBarSubPage) { clip |= subpage; }
        if(controls & SC_ScrollBarAddPage) { clip |= addpage; }
        if(horizontal) { clip |= QRect(slider.x(), 0, slider.width(), sb->height()); }
        else { clip |= QRect(0, slider.y(), sb->width(), slider.height()); }
        clip ^= slider;

        p->setClipRegion(clip);
        Mountain::ScrollBarPainter(MountainGroove1, 2, horizontal).draw(p, slider|subpage|addpage, cg.button(),
                                   cg.background(), disabled);
        ...
        if(controls & SC_ScrollBarSlider) {
          ...
          drawPrimitive(PE_ScrollBarSlider, p, slider, cg, flags | ((active == SC_ScrollBarSlider) ? Style_Down : 0));
        }
        p->setClipping(false);
        ...
        if(controls & (SC_ScrollBarSubLine | SC_ScrollBarAddLine)) {
          ...
          if(active & SC_ScrollBarSubLine) {
            if(horizontal) { p->setClipRect(QRect(addline.x(), addline.y(), addline.width() / 2, addline.height())); }
            else { p->setClipRect(QRect(addline.x(), addline.y(), addline.width(), addline.height() / 2)); }
            drawPrimitive(PE_ScrollBarAddLine, p, addline, cg, flags | Style_Down);
          }
          else if(active & SC_ScrollBarAddLine) {
            if(horizontal) {
              p->setClipRect(QRect(addline.x() + addline.width() / 2, addline.y(), addline.width() / 2,
                             addline.height()));
            }
            else {
              p->setClipRect(QRect(addline.x(), addline.y() + addline.height() / 2, addline.width(),
                             addline.height() / 2));
            }
            drawPrimitive(PE_ScrollBarAddLine, p, addline, cg, flags | Style_Down);
          }
      ...
      case CC_ToolButton: {
        ...
        QRect button = querySubControlMetrics(control, widget, SC_ToolButton, opt);
        QRect menuarea = querySubControlMetrics(control, widget, SC_ToolButtonMenu, opt);
        ...
        if(widget == hoverWidget) { bflags |= Style_MouseOver; }
        ...
        if(controls & SC_ToolButton) {
          ...
          if(bflags & (Style_Down|Style_On|Style_Raised) || onControlButtons) {
            ...
            if(!onToolbar && !onControlButtons) { bflags |= Style_Horizontal; }
          ...
          // Check whether to draw a background pixmap
          ...
          else if(onToolbar) { renderToolbarWidgetBackground(p, widget); }
          else if(onExtender) {
            ...
            int xoff = 0, yoff = 0;
            if(horiz) { yoff = parent->mapToParent(widget->pos()).y(); }
            else { xoff = parent->mapToParent(widget->pos()).x(); }

            Mountain::GradientPainter::renderGradient(p, r, cg.button(), horiz, false, /*Not a menubar*/ xoff,
                                                      yoff, tr.width(), tr.height());
          }
        }
  ...
  void MountainStyle::drawComplexControlMask(ComplexControl control, QPainter *p, const QWidget *widget, const QRect &r,
                                             const QStyleOption& opt) const {
    if(control == CC_ComboBox) {
      maskMode = true;
      drawComplexControl(CC_ComboBox, p, widget, r, QApplication::palette().active(), Style_Default, SC_ComboBoxFrame,
                         SC_None, opt);
      maskMode = false;
    }
    else { p->fillRect(r, color1); }
  }

  int MountainStyle::pixelMetric(PixelMetric m, const QWidget *widget) const {
    switch(m) {
      // BUTTONS
      // -------------------------------------------------------------------
      case PM_ButtonMargin: { return 4; } // Space btw. frame and label
      case PM_SliderLength: { return 12; }
      case PM_SliderControlThickness: { return loader.size(mountain_slider).height() - 4; }
      case PM_SliderThickness: { return loader.size(mountain_slider).height(); }
      case PM_ButtonShiftHorizontal: { return 0; }
      case PM_ButtonShiftVertical: { return 1; } // Offset by 1
      ...
      case PM_ScrollBarSliderMin: {
        return loader.size(mountain_scrollbar_vbar + MountainSlider1).height() + loader.size(mountain_scrollbar_vbar +
            MountainSlider3).height();
      }
      case PM_SpinBoxFrameWidth:
      case PM_DefaultFrameWidth: { return 1; }
      case PM_MenuButtonIndicator: { return 13; }
      ...
      case PM_TitleBarHeight: { return titleBarH; }
      default: { return KStyle::pixelMetric(m, widget); }
    }
  }

  QSize MountainStyle::sizeFromContents(ContentsType contents, const QWidget* widget, const QSize &contentSize,
                                        const QStyleOption& opt) const {
    switch(contents) {
      // PUSHBUTTON SIZE
      // ------------------------------------------------------------------
      case CT_PushButton: {
        ...
        //MX: No longer blank space -- can make a bit smaller
        return QSize(w + 30, h + 5); }
      ...
      case CT_PopupMenuItem: {
        if(! widget || opt.isDefault()) { return contentSize; }
        ...
        if(mi->custom()) {
          ...
          if(! mi->custom()->fullSpan()) { h += 2 * itemVMargin + 2 * itemFrame; }
        }
        else if(mi->widget()) {}
          ...
          if(mi->pixmap()) { h = QMAX(h, mi->pixmap()->height() + 2 * itemFrame); }
          else {
            // Ensure that the minimum height for text-only menu items
            // is the same as the icon size used by KDE.
            h = QMAX(h, 16 + 2 * itemFrame);
            h = QMAX(h, popup->fontMetrics().height() + 2 * itemVMargin + 2 * itemFrame);
          }

          if(mi->iconSet()) {
            h = QMAX(h, mi->iconSet()->pixmap( QIconSet::Small, QIconSet::Normal).height() + 2 * itemFrame);
          }
        }

        if(! mi->text().isNull() && mi->text().find('\t') >= 0) { w += itemHMargin + itemFrame * 2 + 7; }
        else if(mi->popup()) { w += 2 * arrowHMargin; }

        if(maxpmw) { w += maxpmw + 6; }
        if(checkable && maxpmw < 20) { w += 20 - maxpmw; }
        if(checkable || maxpmw > 0) { w += 12; }

        w += rightBorder;

        return QSize(w, h); }

      default: { return KStyle::sizeFromContents(contents, widget, contentSize, opt); }
    }
  }

  QStyle::SubControl MountainStyle::querySubControl(ComplexControl control, const QWidget* widget, const QPoint& point,
                                                    const QStyleOption& opt) const {
  ...
  QRect MountainStyle::querySubControlMetrics(ComplexControl control, const QWidget* widget, SubControl subcontrol,
                                              const QStyleOption& opt) const {
    switch(control) {
      case CC_ComboBox: {
        int arrow;
        bool compact = false;
        if(isSizeConstrainedCombo(static_cast<const QComboBox*>(widget))) { compact = true; } //### constant

        if(compact) { arrow = 11; }
        else { arrow = 11 + loader.size(mountain_ripple).width(); }

        switch(subcontrol) {
          case SC_ComboBoxArrow:
            if(compact) { return QRect(widget->width() - arrow - 7, 0, arrow + 6, widget->height()); }
            else { return QRect(widget->width() - arrow - 14, 0, arrow + 13, widget->height()); }

          case SC_ComboBoxEditField: {
            if(compact) { return QRect(2, 4, widget->width() - arrow - 2 - 7, widget->height() - 8); }
      ...
      case CC_ScrollBar: {
        ...
        if(horizontal) {
          subline = loader.size(mountain_scrollbar_hbar_arrow1).width();
          addline = loader.size(mountain_scrollbar_hbar_arrow2).width();
          maxlen = sb->width() - subline - addline + 2;
        }
        else {
          subline = loader.size(mountain_scrollbar_vbar_arrow1).height();
          addline = loader.size(mountain_scrollbar_vbar_arrow2).height();
        ...
        switch(subcontrol) {
          ...
          case SC_ScrollBarAddPage:
            if(horizontal) {
              return QRect(sliderpos + sliderlen, 0, sb->width() - addline  - (sliderpos + sliderlen) , sb->height());
            }
            else {
              return QRect(0, sliderpos + sliderlen, sb->width(), sb->height() - addline - (sliderpos + sliderlen)
      ...
      case CC_Slider: {
        ...
        if(horizontal) { handleSize = QMIN(handleSize, sl->height()); }
        else { handleSize = QMIN(handleSize, sl->width()); }
        ...
        switch(subcontrol) {
          case SC_SliderGroove:
            if(horizontal) {
              if(ticks == QSlider::Both) { return QRect(0, (sl->height() - size) / 2, sl->width(), size); }
              ...
              if(ticks == QSlider::Both) { return QRect((sl->width() - size) / 2, 0, size, sl->height()); }
              else if(ticks == QSlider::Above) {
          ...
          case SC_SliderHandle:
            if(horizontal) {
              if(ticks == QSlider::Both) { return QRect(pos, (sl->height() - handleSize) / 2, len, handleSize); }
              else if(ticks == QSlider::Above) { return QRect(pos, sl->height() - handleSize, len, handleSize); }
              return QRect(pos, 0, len, handleSize);
            } else {
              if(ticks == QSlider::Both) { return QRect((sl->width() - handleSize) / 2, pos, handleSize, len); }
              else if(ticks == QSlider::Above) { return QRect(sl->width() - handleSize, pos, handleSize, len); }
...
namespace Mountain {
  bool MountainStyle::eventFilter(QObject* object, QEvent* event) {
    ...
    if(event->type() == QEvent::Paint && ::qt_cast<QLineEdit*>(object)) {
      ...
      QPainter p(widget);
      Mountain::RectTilePainter(mountain_frame_shadow, false, false, 2, 2).
          draw(&p, widget->rect(), widget->palette().color(QPalette::Normal, QColorGroup::Button),
               Qt::black, false, Mountain::TilePainter::PaintFullBlend);
      recursion = false;
      return true;
    }
      ...
      switch(event->type()) {
        ...
        case QEvent::Resize: {
          ...
          if(resize->size().height() < 6) { return false; }
    ...
    //Toolbar background gradient handling
    else if(event->type() == QEvent::Paint && object->parent() && !qstrcmp(object->name(), kdeToolbarWidget)) {
      // Draw a gradient background for custom widgets in the toolbar
      ...
      if(toolbar->orientation() == Qt::Horizontal) {
        Mountain::GradientPainter::renderGradient(&p, wr, widget->colorGroup().button(), true /*horizontal*/,
                                                  false /*not a menu*/, 0, widget->y(), wr.width(), tr.height());
      }
      else {
        Mountain::GradientPainter::renderGradient(&p, wr, widget->colorGroup().button(), false /*vertical*/,
                                                  false /*not a menu*/, widget->x(), 0, tr.width(), wr.height());
      }

      //Draw terminator line, too
      p.setPen(toolbar->colorGroup().mid());
      if(toolbar->orientation() == Qt::Horizontal) { p.drawLine(wr.width() - 1, 0, wr.width() - 1, wr.height() - 1); }
      else { p.drawLine(0, wr.height() - 1, wr.width() - 1, wr.height() - 1); }
      return true;
    }

    // Track show events for progress bars
    if(animateProgressBar && ::qt_cast<QProgressBar*>(object)) {
      if((event->type() == QEvent::Show) && !animationTimer->isActive()) { animationTimer->start(50, false); }
    }
