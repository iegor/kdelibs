...
class KFileIconView::KFileIconViewPrivate {
  public:
    KFileIconViewPrivate(KFileIconView *parent) {
      ...
      smallColumns = new KRadioAction(i18n("Small Icons"), 0, parent, SLOT(slotSmallColumns()),
                                      parent->actionCollection(), "small columns");
      largeRows = new KRadioAction(i18n("Large Icons"), 0, parent, SLOT(slotLargeRows()), parent->actionCollection(),
                                   "large rows");
      ...
      previews = new KToggleAction(i18n("Thumbnail Previews"), 0, parent->actionCollection(), "show previews");
      zoomIn = KStdAction::zoomIn(parent, SLOT(zoomIn()), parent->actionCollection(), "zoomIn");
      zoomOut = KStdAction::zoomOut(parent, SLOT(zoomOut()), parent->actionCollection(), "zoomOut");

      previews->setGroup("previews");
      zoomIn->setGroup("previews");
      zoomOut->setGroup("previews");

      connect(previews, SIGNAL(toggled(bool)), parent, SLOT(slotPreviewsToggled(bool)));

      connect(&previewTimer, SIGNAL(timeout()), parent, SLOT(showPreviews()));
      connect(&autoOpenTimer, SIGNAL(timeout()), parent, SLOT(slotAutoOpen()));
    }
...
KFileIconView::KFileIconView(QWidget *parent, const char *name)
  ...
  connect(this, SIGNAL(returnPressed(QIconViewItem *)), SLOT(slotActivate(QIconViewItem *)));

  // we want single click _and_ double click (as convenience)
  connect(this, SIGNAL(clicked(QIconViewItem *, const QPoint&)), SLOT(selected(QIconViewItem *)));
  connect(this, SIGNAL(doubleClicked(QIconViewItem *, const QPoint&)), SLOT(slotActivate(QIconViewItem *)));
  ...
  if(sm == KFile::Multi || sm == KFile::Extended)
    ...
    connect(this, SIGNAL(selectionChanged(QIconViewItem *)), SLOT(highlighted(QIconViewItem *)));
...
void KFileIconView::showPreviews() {
  ...
  connect(d->job, SIGNAL(result(KIO::Job *)), this, SLOT(slotPreviewResult(KIO::Job *)));
  connect(d->job, SIGNAL(gotPreview(const KFileItem*, const QPixmap&)),
          SLOT(gotPreview(const KFileItem*, const QPixmap&)));
  // connect( d->job, SIGNAL( failed( const KFileItem* )), this, SLOT( slotFailed( const KFileItem* ) ));
}

void KFileIconView::slotPreviewResult(KIO::Job *job) {
  if(job == d->job) { d->job = 0L; }
}
...
bool KFileIconView::canPreview(const KFileItem *item) const {
  ...
  for(; it != d->previewMimeTypes.end(); ++it) {
    ...
    else if(item->mimetype() == type) { return true; }
  }
...
KFileItem * KFileIconView::nextItem(const KFileItem *fileItem) const {
  if(fileItem) {
    ...
    if(item && item->nextItem()) { return ((KFileIconViewItem*) item->nextItem())->fileInfo(); }
  }
  return 0L;
}
...
KFileItem * KFileIconView::prevItem(const KFileItem *fileItem) const {
  if(fileItem) {
    KFileIconViewItem *item = viewItem(fileItem);
    if(item && item->prevItem()) { return ((KFileIconViewItem*) item->prevItem())->fileInfo(); }
  }
  return 0L;
}
